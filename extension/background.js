...
import {compileInstruction, normalizeInstruction} from './rule-compiler.js';
...
async function saveInstruction(message) {
  ...
  try {
    ...
    if (s.filter?.instruction === instruction) return {ok: true, instruction,
      filterSummary: s.filter.summary, filterId: s.filter.id};
    if (!s.openaiApiKey) throw new Error('Add your OpenAI API key in Settings to save an instruction.');
    timer = setTimeout(() => controller.abort(), 25000);
    const compiled = await compileInstruction(instruction, s.openaiApiKey, controller.signal);
    current();
    const question = normalizeQuestion(compiled.question);
    const filter = {...compiled, question, id: await hash(JSON.stringify({model: MODEL, instruction, question}))};
